...
<div class="max-w-3xl mx-auto bg-white border-collapse border-2 border-indigo-400 rounded-lg shadow-lg p-6">
    ...
    <form method="POST" action="{{route('register.step', ['step'=>8])}}" >
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <div class="flex justify-between gap-4">
                    <div class="w-1/2 flex">
                        <input type="radio" id="monthly" name="payment" value="monthly" class="hidden peer" onchange="updateValue()" />
                        <label for="monthly"
                    ...
                    <div class="w-1/2 flex">
                        <input type="radio" id="yearly" name="payment" value="yearly" class="hidden peer" checked onchange="updateValue()" />
                        <label for="yearly"
                            class="block w-full border-2 border-gray-300 bg-gray-50 rounded-lg p-4 text-center cursor-pointer peer-checked:border-indigo-500 peer-checked:bg-indigo-100">
            ...
            <div class="bg-gray-50 rounded-lg p-4 space-y-6">
                ...
                @error('cardnum')
                    <span class="text-red-600 text-sm">{{$message}}</span><br>
                @enderror
                @error('expiry')
                    <span class="text-red-600 text-sm">{{$message}}</span><br>
                @enderror
 ...
 <script>
    ...
    function updateValue() {
        ...
        if (isMonthly) {
            ...
            element.textContent = {{ $data['mantant'] }};
            });

        } else {
            valueElements.forEach(element => {
            element.textContent = {{number_format( $data['mantant'] * 12 ,2, '.', '')}};
            });
        }
    }
